...
static void Decode(void)  /* recover */
{
    ...
    textsize =  (((unsigned long)fgetc(infile)));
...
unsigned char
decompress_file(char *ifname,char *ofname, void (*errf)(char *))
{
   textsize = 0;
   codesize = 0;
   getbuf = 0;
...
int PrintFormat( int ErrorLevel )
{
     printf( "\n" );
     printf( "%s.EXE v%s - Huffman Compression/Decompression Utility\n", PROGRAM_NAME, VERSION_NUM );
     printf( "Copyright (c) 1999  TeleGrafix Communications, Inc.  All Rights Reserved\n" );
     printf( "\n" );
     printf( "Format:  %s <-?CD> <input-file> <output-file>\n", PROGRAM_NAME );
     printf( "\n" );
     printf( "Where:    -? ... Display this help screen\n" );
     printf( "          -C ... Compress   the 'input-file', and write to 'output-file'\n" );
     printf( "          -D ... Decompress the 'input-file', and write to 'output-file'\n" );
     printf( "\n" );
     printf( "This program is intended only to (de)compress a single file.  Wildcards are\n" );
     printf( "not supported.  The output data is written 'raw', with no special header\n" );
     printf( "information.\n" );
                 ...
                 char *argv[] )
{
     for ( int iArg = 1;  iArg < argc;  iArg++ )
     {
               ...
               argv[ iArg ] [ 0 ]  ==  '/' )
          {
               ...
               for ( int iChar = 1;  iChar < strlen( argv[ iArg ] );  iChar++ )
               {
                    switch ( toupper( argv[ iArg ] [ iChar ] ) )
                    {
                         case 'C':
          ...
          else if ( pInFilename == NULL )
          {
               pInFilename  =  argv[ iArg ];
          }
          else if ( pOutFilename == NULL )
          {
               pOutFilename  =  argv[ iArg ];
          }
          else
          {
               return  PrintFormat( 1 );
...
//**************************************************************************
//
// main() routine for Huffman compression/decompression program
//
//     Prototype:  int main( int argc, char *argv[] );
//
//    Parameters:  argc ... Number of command-line arguments
//                 argv ... Table  of command-line arguments
//
//       Returns:  0 upon success, or 1 on failure.
//
//       Remarks:
//
// 
...
// -------------------------------------------------------------------------
//
//    Created on:  12/10/99 (Jeff Reeder)
// Last modified:  12/10/99 (Jeff Reeder)   Created
//
//**************************************************************************
...
int main( int   argc,
          char *argv[] )
{
     unsigned long cInSize;
     unsigned long cOutSize;
     ...
     if ( iMode == MODE_COMPRESS )
     {
          ...
          if ( compress_file( pInFilename, 
                              pOutFilename, 
                              cInSize,
                              cOutSize,
                              MyErrorFn ) )
          {
               long iPer  =  ( cOutSize * 100L )  /  cInSize;

               printf( "(%ld%%)", iPer );
          }
     }
     else if ( iMode == MODE_DECOMPRESS )
     {
          printf( "%13.13s: %-12s -> %s ",
                    "Decompressing",
                    pInFilename,
